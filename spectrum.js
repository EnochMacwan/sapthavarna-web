export class SpectrumAnimation {
    ...
    initBrandBuild() {
        ...
        for(let y=0; y<h; y+=gap) for(let x=0; x<w; x+=gap) if(data[(y*w+x)*4+3]>128) {
            this.particles.push({ tx:cw/2-w/2+x, ty:ch/2-h/2+y, x:Math.random()*cw, y:Math.random()*ch, c:Math.random()>0.5?'rgba(156,66,33,0.8)':'rgba(45,55,72,0.8)', d:Math.random()*60 });
        }
    }
    ...
    drawStructuralAssembly() { this.ctx.strokeStyle='rgba(45,55,72,0.2)'; this.particles.forEach(p=>{ if(p.progress<1)p.progress+=0.01; const y=-500+(p.targetY+500)*p.progress; this.ctx.strokeRect(p.x,y,p.w,15); }); }
    ...
    drawHydroFlow() { const t=Date.now()*0.001; this.ctx.strokeStyle='rgba(0,51,102,0.1)'; this.particles.forEach(p=>{ const a=Math.sin(p.x*0.01+t)*Math.cos(p.y*0.01+t)*Math.PI; this.ctx.beginPath(); this.ctx.moveTo(p.x,p.y); this.ctx.lineTo(p.x+Math.cos(a)*p.l, p.y+Math.sin(a)*p.l); this.ctx.stroke(); }); }
    drawEmSignal() { const t=Date.now()*0.01; this.ctx.strokeStyle='rgba(0,51,102,0.1)'; this.particles.nodes.forEach(n=>{ this.ctx.beginPath(); for(let i=0;i<20;i++)this.ctx.lineTo(n.x+(i-10)*12, n.y+Math.sin(t+i*0.5)*20); this.ctx.stroke(); }); }
    drawSoilCompaction() { const t=Date.now()*0.04; this.particles.forEach(p=>{ p.y+=(p.ty-p.y)*0.01; this.ctx.fillStyle='rgba(45,55,72,0.15)'; this.ctx.beginPath(); this.ctx.arc(p.x+Math.sin(t+p.x)*2, p.y, 1.5,0,Math.PI*2); this.ctx.fill(); }); }
    drawGeoAnnotate() { const w=this.canvas.width/this.dpr, h=this.canvas.height/this.dpr; this.ctx.strokeStyle='rgba(156,66,33,0.1)'; this.ctx.setLineDash([5,5]); this.ctx.beginPath(); this.ctx.moveTo(this.mouse.x,0); this.ctx.lineTo(this.mouse.x,h); this.ctx.moveTo(0,this.mouse.y); this.ctx.lineTo(w,this.mouse.y); this.ctx.stroke(); this.ctx.setLineDash([]); this.ctx.fillStyle='rgba(156,66,33,0.3)'; this.ctx.font='10px Inter'; this.ctx.fillText(`X:${this.mouse.x.toFixed(0)}`, this.mouse.x+10, 20); this.ctx.fillText(`Y:${(h-this.mouse.y).toFixed(0)}`, 10, this.mouse.y-10); }
    drawVectorField() { this.ctx.strokeStyle='rgba(0,51,102,0.05)'; this.particles.forEach(p=>{ const a = Math.atan2(this.mouse.y-p.y, this.mouse.x-p.x); this.ctx.save(); this.ctx.translate(p.x, p.y); this.ctx.rotate(a); this.ctx.beginPath(); this.ctx.moveTo(-5,0); this.ctx.lineTo(5,0); this.ctx.lineTo(2,2); this.ctx.stroke(); this.ctx.restore(); }); }
    drawPointMesh() { this.ctx.fillStyle='rgba(0,51,102,0.2)'; this.ctx.strokeStyle='rgba(0,51,102,0.03)'; this.particles.forEach((p,i)=>{ p.x+=p.vx; p.y+=p.vy; if(p.x<0||p.x>this.canvas.width/this.dpr)p.vx*=-1; if(p.y<0||p.y>this.canvas.height/this.dpr)p.vy*=-1; this.ctx.beginPath(); this.ctx.arc(p.x,p.y,1.5,0,Math.PI*2); this.ctx.fill(); for(let j=i+1; j<this.particles.length; j++){ const p2=this.particles[j], d=Math.sqrt((p.x-p2.x)**2+(p.y-p2.y)**2); if(d < 60) { this.ctx.beginPath(); this.ctx.moveTo(p.x, p.y); this.ctx.lineTo(p2.x, p2.y); this.ctx.stroke(); } } }); }
    drawStrainGauge() { this.particles.forEach(p=>{ const dx=p.bx-this.mouse.x, dy=p.by-this.mouse.y, d=Math.sqrt(dx*dx+dy*dy); const move=d<200 ? (200-d)*0.15 : 0; p.x=p.bx+ (dx/d||0)*move; p.y=p.by+ (dy/d||0)*move; this.ctx.fillStyle=d<150?`rgba(156,66,33,${1-d/150})`:'rgba(0,51,102,0.05)'; this.ctx.beginPath(); this.ctx.arc(p.x, p.y, 1, 0, Math.PI*2); this.ctx.fill(); }); }
    drawTopoHUD() { const t=Date.now()*0.001; this.ctx.strokeStyle='rgba(156,66,33,0.1)'; this.ctx.font='9px monospace'; this.particles.forEach(p=>{ this.ctx.beginPath(); this.ctx.arc(this.canvas.width/this.dpr/2, this.canvas.height/this.dpr/2, p.r+Math.sin(t+p.offset)*20, 0, Math.PI*2); this.ctx.stroke(); this.ctx.fillStyle='rgba(156,66,33,0.2)'; this.ctx.fillText(`ELV:+${(p.r/10).toFixed(0)}m`, this.canvas.width/this.dpr/2 + p.r, this.canvas.height/this.dpr/2); }); }
    drawBlueprintGhost() { const t=Date.now()*0.0005; this.ctx.strokeStyle='rgba(0,51,102,0.05)'; this.ctx.setLineDash([10,5]); this.particles.forEach(p=>{ const s=0.8+Math.sin(t+p.phase)*0.2; this.ctx.strokeRect(p.x-p.w*s/2, p.y-p.h*s/2, p.w*s, p.h*s); this.ctx.beginPath(); this.ctx.moveTo(p.x-p.w*s/2, p.y); this.ctx.lineTo(p.x+p.w*s/2, p.y); this.ctx.stroke(); }); this.ctx.setLineDash([]); }
    drawCoordAxis() { const w=this.canvas.width/this.dpr, h=this.canvas.height/this.dpr; this.particles.rot+=0.01; this.ctx.strokeStyle='rgba(45,55,72,0.1)'; this.ctx.save(); this.ctx.translate(w/2, h/2); this.ctx.rotate(this.particles.rot); ['rgba(156,66,33,0.2)','rgba(0,51,102,0.2)','rgba(45,55,72,0.2)'].forEach((c,i)=>{ this.ctx.strokeStyle=c; this.ctx.beginPath(); this.ctx.moveTo(0,0); this.ctx.lineTo(150*Math.cos(i*2.1), 150*Math.sin(i*2.1)); this.ctx.stroke(); this.ctx.fillStyle=c; this.ctx.fillText(['X-AXIS','Y-AXIS','Z-AXIS'][i], 160*Math.cos(i*2.1), 160*Math.sin(i*2.1)); }); this.ctx.restore(); }
    drawHatchProfile() { this.particles.forEach(p=>{ this.ctx.strokeStyle='rgba(45,55,72,0.05)'; for(let i=0; i<p.h; i+=10){ this.ctx.beginPath(); this.ctx.moveTo(0, p.y+i); this.ctx.lineTo(this.canvas.width/this.dpr, p.y+i+ (p.type===0?100:-100)); this.ctx.stroke(); } this.ctx.fillStyle='rgba(45,55,72,0.05)'; this.ctx.fillText(['SILT','CLAY','SHALE'][p.type], 20, p.y+20); }); }
    drawTriangulation() { this.ctx.strokeStyle='rgba(156,66,33,0.1)'; this.ctx.setLineDash([2,2]); this.particles.anchors.forEach(a=>{ this.ctx.beginPath(); this.ctx.moveTo(a.x, a.y); this.ctx.lineTo(this.mouse.x, this.mouse.y); this.ctx.stroke(); const d=Math.sqrt((a.x-this.mouse.x)**2+(a.y-this.mouse.y)**2); this.ctx.fillStyle='rgba(156,66,33,0.2)'; this.ctx.font='9px monospace'; this.ctx.fillText(`DIST:${d.toFixed(0)}m`, (a.x+this.mouse.x)/2, (a.y+this.mouse.y)/2); }); this.ctx.setLineDash([]); }
    drawFluidStream() { const w=this.canvas.width/this.dpr, t=Date.now()*0.002; this.ctx.strokeStyle='rgba(0,51,102,0.05)'; this.particles.forEach(p=>{ this.ctx.beginPath(); this.ctx.moveTo(0, p.y); for(let x=0; x<w; x+=20) this.ctx.lineTo(x, p.y + Math.sin(x*0.01+t+p.offset)*30); this.ctx.stroke(); const ax=(t*200+p.offset*100)%w; const ay=p.y+Math.sin(ax*0.01+t+p.offset)*30; this.ctx.fillStyle='rgba(0,51,102,0.2)'; this.ctx.beginPath(); this.ctx.arc(ax,ay,2,0,Math.PI*2); this.ctx.fill(); }); }
    drawTrussLogic() { this.ctx.strokeStyle='rgba(45,55,72,0.1)'; this.particles.forEach((p,i)=>{ this.ctx.beginPath(); this.ctx.moveTo(p.x, p.yTop); this.ctx.lineTo(p.x, p.yBot); this.ctx.stroke(); if(this.particles[i+1]){ const p2=this.particles[i+1]; this.ctx.beginPath(); this.ctx.moveTo(p.x,p.yTop); this.ctx.lineTo(p2.x,p2.yTop); this.ctx.lineTo(p2.x,p2.yBot); this.ctx.lineTo(p.x,p.yTop); this.ctx.stroke(); } }); }
    drawTruckMove() { const w=this.canvas.width/this.dpr; this.ctx.fillStyle='rgba(45,55,72,0.15)'; this.particles.forEach(p=>{ p.x+=p.s; if(p.x>w+100)p.x=-100; this.ctx.fillRect(p.x,p.y,40,20); this.ctx.fillRect(p.x+30,p.y-10,15,15); this.ctx.beginPath(); this.ctx.arc(p.x+10,p.y+20,5,0,Math.PI*2); this.ctx.arc(p.x+35,p.y+20,5,0,Math.PI*2); this.ctx.fill(); }); }
    drawCraneLift() { const w=this.canvas.width/this.dpr, h=this.canvas.height/this.dpr; const t=Date.now()*0.001; const ly=Math.sin(t)*50; this.ctx.strokeStyle='rgba(0,51,102,0.2)'; this.ctx.beginPath(); this.ctx.moveTo(w/2,h); this.ctx.lineTo(w/2,h*0.3); this.ctx.lineTo(w/2+200,h*0.3); this.ctx.stroke(); this.ctx.beginPath(); this.ctx.moveTo(w/2+180,h*0.3); this.ctx.lineTo(w/2+180,h*0.3+100+ly); this.ctx.stroke(); this.ctx.strokeRect(w/2+155,h*0.3+100+ly,50,20); }
    drawEarthScoop() { const t=Date.now()*0.002, bx=this.canvas.width/this.dpr/2+Math.cos(t)*200, by=this.canvas.height/this.dpr*0.7+Math.sin(t*2)*50; if(Math.sin(t*2)>0.8 && Math.random()<0.3) this.particles.dirt.push({x:bx, y:by, vy:0}); this.ctx.fillStyle='rgba(156,66,33,0.2)'; this.particles.dirt.forEach((d,i)=>{ d.vy+=0.2; d.y+=d.vy; if(d.y>this.canvas.height/this.dpr)this.particles.dirt.splice(i,1); this.ctx.beginPath(); this.ctx.arc(d.x,d.y,3,0,Math.PI*2); this.ctx.fill(); }); this.ctx.strokeStyle='rgba(45,55,72,0.3)'; this.ctx.strokeRect(bx-20,by-20,40,30); }
    drawConcretePump() { const w=this.canvas.width/this.dpr, h=this.canvas.height/this.dpr, t=Date.now()*0.02; this.ctx.strokeStyle='rgba(45,55,72,0.2)'; this.ctx.lineWidth=8; this.ctx.beginPath(); this.ctx.moveTo(w*0.2,h); this.ctx.lineTo(w*0.3,h*0.5); this.ctx.lineTo(w*0.5,h*0.4); this.ctx.stroke(); this.ctx.lineWidth=1; if(Math.random()<0.2) this.particles.drips.push({x:w*0.5, y:h*0.4, s:Math.random()*2+2}); this.ctx.fillStyle='rgba(45,55,72,0.15)'; this.particles.drips.forEach((d,i)=>{ d.y+=d.s; if(d.y>h)this.particles.drips.splice(i,1); this.ctx.beginPath(); this.ctx.arc(d.x,d.y,4,0,Math.PI*2); this.ctx.fill(); }); }
    drawJackHammer() { const x=this.canvas.width/this.dpr/2+(Math.random()-0.5)*10, y=this.canvas.height/this.dpr/2+(Math.random()-0.5)*10; if(Math.random()<0.5) this.particles.sparks.push({x,y, vx:(Math.random()-0.5)*10, vy:(Math.random()-0.5)*10, a:1}); this.ctx.strokeStyle='rgba(45,55,72,0.3)'; this.ctx.strokeRect(x-5,y-40,10,80); this.particles.sparks.forEach((s,i)=>{ s.x+=s.vx; s.y+=s.vy; s.a-=0.05; if(s.a<=0)this.particles.sparks.splice(i,1); this.ctx.fillStyle=`rgba(156,66,33,${s.a})`; this.ctx.beginPath(); this.ctx.arc(s.x,s.y,2,0,Math.PI*2); this.ctx.fill(); }); }
    drawRoadPaver() { const w=this.canvas.width/this.dpr, h=this.canvas.height/this.dpr; this.particles.x=(this.particles.x+2)%(w+200); this.particles.trail.push({x:this.particles.x-100, y:h*0.6}); if(this.particles.trail.length>200)this.particles.trail.shift(); this.ctx.fillStyle='rgba(45,55,72,0.1)'; this.particles.trail.forEach(pt=>this.ctx.fillRect(pt.x,pt.y,10,50)); this.ctx.fillStyle='rgba(45,55,72,0.3)'; this.ctx.fillRect(this.particles.x-100,h*0.6-10,100,70); }
    drawActiveDrone() { const w=this.canvas.width/this.dpr, h=this.canvas.height/this.dpr; const t=Date.now()*0.001; const x=w/2+Math.cos(t)*300, y=h/2+Math.sin(t*0.5)*200; this.ctx.strokeStyle='rgba(0,51,102,0.3)'; this.ctx.beginPath(); this.ctx.moveTo(x-30,y); this.ctx.lineTo(x+30,y); this.ctx.moveTo(x,y-30); this.ctx.lineTo(x,y+30); this.ctx.stroke(); this.ctx.fillStyle=`rgba(156,66,33,${0.15+Math.sin(t*10)*0.05})`; this.ctx.beginPath(); this.ctx.arc(x,y,80,0,Math.PI*2); this.ctx.fill(); }
    drawScaffoldUp() { this.particles.forEach(p=>{ if(Math.random()<0.01)p.active=1; if(p.active>0){ p.y+=(p.ty-p.y)*0.05; p.active-=0.005; this.ctx.strokeStyle=`rgba(45,55,72,${p.active*0.3})`; this.ctx.strokeRect(p.x,p.y,40,10); this.ctx.beginPath(); this.ctx.moveTo(p.x,p.y); this.ctx.lineTo(p.x,this.canvas.height/this.dpr); this.ctx.stroke(); } }); }
    drawTrafficZone() { const w=this.canvas.width/this.dpr, h=this.canvas.height/this.dpr; this.particles.forEach(p=>{ p.x+=p.s; if(p.x>w+50)p.x=-50; this.ctx.fillStyle='rgba(0,51,102,0.15)'; this.ctx.fillRect(p.x,p.y,30,15); }); this.ctx.fillStyle='rgba(156,66,33,0.3)'; for(let i=0;i<w;i+=100){ this.ctx.beginPath(); this.ctx.moveTo(i,h*0.5-20); this.ctx.lineTo(i-10,h*0.5+20); this.ctx.lineTo(i+10,h*0.5+20); this.ctx.fill(); } }
    drawMasonryBuild() { this.particles.forEach(p=>{ if(Math.random()<0.005)p.active=1; if(p.active>0){ this.ctx.fillStyle=`rgba(45,55,72,${p.active*0.25})`; this.ctx.fillRect(p.x,p.y,38,18); p.active-=0.002; } }); }

    // --- SIGNATURE DRAW ---
    drawBrandBuild() {
        this.ctx.fillStyle = 'rgba(255, 255, 255, 0.05)'; // Ghost text background
