...
`).join(""),q=e=>e.map(t=>`<li>${t}</li>`).join(""),o={home:()=>{const e=l();return`
    ...
    `},teamDetail:(e,t)=>{const i=l(),a=(window.TEAM_DATA||i.about?.leadershipTeam||[]).find(n=>n.name.toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"")===e)||{name:"Team Member Not Found",role:"Unknown",desc:"The requested team member could not be found.",bio:"",photo:""};return`
        ...
        <section id="team-detail">
            <div class="team-detail-card nm-card">
                <div class="team-detail-photo">
                    ${a.photo?`<img src="${v(a.photo)}" alt="${a.name}">`:`<div class="team-avatar-xl">${a.name.charAt(0)}</div>`}
                </div>
    ...
    `};function W(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=C.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=C.footer(),I()}function I(){const e=window.location.hash.replace("#","").split("?")[0];document.querySelectorAll(".nav-link").forEach(t=>{const i=t.getAttribute("href")||"",s=i.startsWith("#")?i.substring(1):"";e===s||e===""&&s===""?t.classList.add("active"):t.classList.remove("active")})}let u=null;T.initialize({startOnLoad:!1,theme:"base",themeVariables:{primaryColor:"#f6f5f2",primaryTextColor:"#1c1c1c",primaryBorderColor:"rgba(0,0,0,0.1)",lineColor:"#C69061",secondaryColor:"#f6f5f2",tertiaryColor:"#f6f5f2"}});const E=()=>{u&&(u.destroy(),u=null),document.getElementById("spectrum-canvas")&&(u=new D("spectrum-canvas"));const t={threshold:.05},i=new IntersectionObserver(a=>{a.forEach(n=>{n.isIntersecting&&(gsap.to(n.target,{opacity:1,y:0,duration:.8,ease:"power2.out"}),i.unobserve(n.target))})},t);document.querySelectorAll(".nm-card, h2, #closing h2, .subpage-hero h1, .hero-content h1").forEach(a=>{a.style.opacity="0",a.style.transform="translateY(15px)",i.observe(a)}),document.querySelectorAll(".mermaid").length>0&&T.run(),I()},B=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const i=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(i,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{W();const e=document.getElementById("app");new R(e),B(),window.addEventListener("routeChange",()=>{E()}),E()});window.setAnimationMode=e=>{u&&typeof u.setMode=="function"&&(u.setMode(e),document.querySelectorAll(".sample-btn").forEach(t=>{t.getAttribute("onclick").match(/'([^']+)'/)[1]===e?t.classList.add("active"):t.classList.remove("active")}))};window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,i=new FormData(t),s=Object.fromEntries(i.entries());s.submittedAt=new Date().toISOString();const a=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");a.push(s),localStorage.setItem("eoi_submissions",JSON.stringify(a)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"}),console.log("EOI Submitted:",s)};const H=()=>{const e=document.createElement("button");e.className="back-to-top",e.setAttribute("aria-label","Back to top"),e.innerHTML='<i class="fas fa-arrow-up"></i>',document.body.appendChild(e),e.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}),window.addEventListener("scroll",()=>{window.scrollY>400?e.classList.add("visible"):e.classList.remove("visible")})},V=()=>{const e=document.querySelector("nav");if(!e)return;const t=document.createElement("div");t.className="hamburger",t.innerHTML="<span></span><span></span><span></span>",t.setAttribute("aria-label","Toggle menu");const i=document.createElement("div");i.className="mobile-nav",i.innerHTML=`
        ...
        <a href="#contact" class="nav-link">Contact</a>
    `;const s=e.querySelector(".nav-container");s&&s.appendChild(t),document.body.appendChild(i),t.addEventListener("click",()=>{t.classList.toggle("active"),i.classList.toggle("active"),document.body.style.overflow=i.classList.contains("active")?"hidden":""}),i.querySelectorAll("a").forEach(a=>{a.addEventListener("click",()=>{t.classList.remove("active"),i.classList.remove("active"),document.body.style.overflow=""})})},F=()=>{const e=document.createElement("button");e.className="theme-toggle",e.setAttribute("aria-label","Toggle dark mode"),e.innerHTML='<i class="fas fa-moon"></i>',document.body.appendChild(e),localStorage.getItem("theme")==="dark"&&(document.body.classList.add("dark-mode"),e.innerHTML='<i class="fas fa-sun"></i>'),e.addEventListener("click",()=>{document.body.classList.toggle("dark-mode");const i=document.body.classList.contains("dark-mode");localStorage.setItem("theme",i?"dark":"light"),e.innerHTML=i?'<i class="fas fa-sun"></i>':'<i class="fas fa-moon"></i>'})},M=()=>{const e=document.querySelectorAll(".stat-number");if(e.length===0)return;const t=s=>{const a=parseInt(s.getAttribute("data-target"))||0,n=s.getAttribute("data-suffix")||"",r=2e3,c=0,f=performance.now(),m=p=>{const g=p-f,d=Math.min(g/r,1),h=1-Math.pow(1-d,3),b=Math.floor(c+(a-c)*h);s.textContent=b.toLocaleString()+n,d<1&&requestAnimationFrame(m)};requestAnimationFrame(m)},i=new IntersectionObserver(s=>{s.forEach(a=>{a.isIntersecting&&!a.target.classList.contains("animated")&&(a.target.classList.add("animated"),t(a.target))})},{threshold:.5});e.forEach(s=>i.observe(s))},L=()=>{document.querySelectorAll(".form-input").forEach(e=>{e.addEventListener("blur",()=>{if(e.required&&!e.value.trim())e.classList.add("invalid"),e.classList.remove("valid");else if(e.type==="email"&&e.value){const t=/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.value);e.classList.toggle("valid",t),e.classList.toggle("invalid",!t)}else e.value.trim()&&(e.classList.add("valid"),e.classList.remove("invalid"))}),e.addEventListener("input",()=>{e.classList.remove("invalid")})})};document.addEventListener("DOMContentLoaded",()=>{H(),V(),F(),M(),L()});window.addEventListener("routeChange",()=>{M(),L()});
