...
`).join(""),P=e=>e.map(t=>`
    <div class="grid-card-wrapper">
        <div class="grid-card-visual">
            ${t.photo?`<img src="${h(t.photo)}" alt="${t.name}" loading="lazy">`:`<div class="team-avatar-large">${t.name.charAt(0)}</div>`}
        </div>
...
`).join(""),O=e=>e.map(t=>`<li>${t}</li>`).join(""),o={home:()=>{const e=c();return`
        ...
        <section id="about">
            <div class="about-grid">
                ...
                <div class="nm-card img-card nm-inset">
                    <img src="${h("materials.png")}" alt="Engineering materials" class="brand-img">
                </div>
            </div>
        </section>
        ...
        <section id="project-detail">
            <div class="detail-grid">
                <div class="detail-main">
                    ${i.image?`<div class="nm-card detail-image nm-inset"><img src="${h(i.image)}" alt="${i.title}"></div>`:""}
        ...
        <section id="team-detail">
            <div class="team-detail-card nm-card">
                <div class="team-detail-photo">
                    ${s.photo?`<img src="${h(s.photo)}" alt="${s.name}">`:`<div class="team-avatar-xl">${s.name.charAt(0)}</div>`}
                </div>
    ...
    `};function q(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=$.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=$.footer(),E()}function E(){const e=window.location.pathname;document.querySelectorAll(".nav-link").forEach(t=>{const a=new URL(t.href,window.location.origin).pathname;a===e||e==="/"&&a==="/"?t.classList.add("active"):t.classList.remove("active")})}let d=null;C.initialize({startOnLoad:!1,theme:"base",themeVariables:{primaryColor:"#f6f5f2",primaryTextColor:"#1c1c1c",primaryBorderColor:"rgba(0,0,0,0.1)",lineColor:"#C69061",secondaryColor:"#f6f5f2",tertiaryColor:"#f6f5f2"}});const S=()=>{d&&(d.destroy(),d=null),document.getElementById("spectrum-canvas")&&(d=new L("spectrum-canvas"));const t={threshold:.05},a=new IntersectionObserver(s=>{s.forEach(n=>{n.isIntersecting&&(gsap.to(n.target,{opacity:1,y:0,duration:.8,ease:"power2.out"}),a.unobserve(n.target))})},t);document.querySelectorAll(".nm-card, h2, #closing h2, .subpage-hero h1, .hero-content h1").forEach(s=>{s.style.opacity="0",s.style.transform="translateY(15px)",a.observe(s)}),document.querySelectorAll(".mermaid").length>0&&C.run(),E()},D=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const a=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(a,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{q();const e=document.getElementById("app");new R(e),D(),window.addEventListener("routeChange",()=>{S()}),S()});window.setAnimationMode=e=>{d&&typeof d.setMode=="function"&&(d.setMode(e),document.querySelectorAll(".sample-btn").forEach(t=>{t.getAttribute("onclick").match(/'([^']+)'/)[1]===e?t.classList.add("active"):t.classList.remove("active")}))};window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,a=new FormData(t),i=Object.fromEntries(a.entries());i.submittedAt=new Date().toISOString();const s=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");s.push(i),localStorage.setItem("eoi_submissions",JSON.stringify(s)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"}),console.log("EOI Submitted:",i)};
