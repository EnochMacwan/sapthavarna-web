...
`).join(""),P=e=>e.map(t=>`
    <div class="grid-card-wrapper">
        <div class="grid-card-visual">
            ${t.photo?`<img src="${u(t.photo)}" alt="${t.name}" loading="lazy">`:`<div class="team-avatar-large">${t.name.charAt(0)}</div>`}
        </div>
...
`).join(""),O=e=>e.map(t=>`<li>${t}</li>`).join(""),o={home:()=>{const e=c();return`
        ...
        <section id="about">
            <div class="about-grid">
                ...
                <div class="nm-card img-card nm-inset">
                    <img src="${u("materials.png")}" alt="Engineering materials" class="brand-img">
                </div>
            </div>
        </section>
        ...
        <section id="project-detail">
            <div class="detail-grid">
                <div class="detail-main">
                    ${i.image?`<div class="nm-card detail-image nm-inset"><img src="${u(i.image)}" alt="${i.title}"></div>`:""}
                    
                    <div class="nm-card">
                        <h2 class="mb-4">Project Overview</h2>
                        <p class="text-secondary mb-6">${i.description}</p>
                        
                        ${i.details.length>0?`
                        <h4 class="mb-4">Key Features</h4>
                        <ul class="detail-features">
                            ${i.details.map(s=>`<li><i class="fas fa-check"></i> ${s}</li>`).join("")}
                        </ul>
                        `:""}
                    </div>
                </div>
                ...
                <aside class="detail-sidebar">
                    <div class="nm-card">
                        ...
                        <div class="info-item">
                            ...
                            <span class="info-value">${i.year}</span>
                        </div>
                        <div class="info-item">
                            <span class="info-label">Client</span>
                            <span class="info-value">${i.client}</span>
                        </div>
                    </div>
            ...
            <div class="detail-content-wide">
                ...
                <div class="nm-card mt-4">
                    ...
                    <div class="sector-tags">
                        ${i.sectors.map(s=>`<span class="sector-tag">${s}</span>`).join("")}
                    </div>
                </div>
            </div>
        </section>
    ...
    `},teamDetail:(e,t)=>{const a=c(),s=(window.TEAM_DATA||a.about?.team||[]).find(n=>n.name.toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"")===e)||{name:"Team Member Not Found",role:"Unknown",desc:"The requested team member could not be found.",bio:"",photo:""};return`
        ...
        <section id="team-detail">
            <div class="team-detail-card nm-card">
                <div class="team-detail-photo">
                    ${s.photo?`<img src="${u(s.photo)}" alt="${s.name}">`:`<div class="team-avatar-xl">${s.name.charAt(0)}</div>`}
                </div>
                <div class="team-detail-info">
                    <h1>${s.name}</h1>
                    <h3 class="text-accent">${s.role}</h3>
                    <p class="text-secondary mt-4">${s.desc}</p>
                    ${s.bio?`<p class="mt-4">${s.bio}</p>`:""}
                    
                    ${s.linkedin||s.email?`
                    <div class="team-contact mt-6">
                        ${s.email?`<a href="mailto:${s.email}" class="contact-link"><i class="fas fa-envelope"></i> Email</a>`:""}
                        ${s.linkedin?`<a href="${s.linkedin}" class="contact-link" target="_blank"><i class="fab fa-linkedin"></i> LinkedIn</a>`:""}
                    </div>
                    `:""}
                </div>
            </div>
    ...
    `}};class R{constructor(t){this.mountPoint=t,this.routes={"":o.home,"/":o.home,about:o.about,capabilities:o.capabilities,marine:o.marine,transport:o.transport,energy:o.energy,systems:o.systems,sustainability:o.sustainability,contact:o.contact,careers:o.careers},this.dynamicRoutes={project:o.projectDetail,service:o.serviceDetail,team:o.teamDetail},window.addEventListener("hashchange",()=>this.handleRoute()),document.addEventListener("click",a=>this.interceptClick(a)),this.handleRoute()}interceptClick(t){const a=t.target.closest("a");if(a&&a.classList.contains("nav-link")){const i=a.getAttribute("href");if(i&&i.startsWith("/")&&!i.startsWith("//")){t.preventDefault();const s=i==="/"?"":i.substring(1);window.location.hash=s}}}navigate(t){window.location.hash=t}getHashPath(){let t=window.location.hash.substring(1);return t.startsWith("/")&&(t=t.substring(1)),t}getQueryParams(t){const a={},i=t.indexOf("?");return i>-1&&t.substring(i+1).split("&").forEach(n=>{const[r,l]=n.split("=");a[decodeURIComponent(r)]=decodeURIComponent(l||"")}),a}async handleRoute(){let t=this.getHashPath();const a=this.getQueryParams(t),i=t.indexOf("?");i>-1&&(t=t.substring(0,i));let s;const n=Object.keys(this.dynamicRoutes).find(l=>t.startsWith(l));n&&a.id?s=()=>this.dynamicRoutes[n](a.id,a):s=this.routes[t]||o.home;const r=this.mountPoint;window.gsap&&await window.gsap.to(r,{opacity:0,y:10,duration:.3,ease:"power2.in"}),r.innerHTML=s(),window.scrollTo(0,0),window.gsap&&window.gsap.fromTo(r,{opacity:0,y:10},{opacity:1,y:0,duration:.5,ease:"power2.out"}),window.dispatchEvent(new CustomEvent("routeChange",{detail:{path:t,queryParams:a}}))}}class L{constructor(t){this.canvas=document.getElementById(t),this.canvas&&(this.ctx=this.canvas.getContext("2d",{alpha:!0}),this.particles=[],this.sparks=[],this.worker={x:0,y:0,tx:0,ty:0,state:"idle",targetIdx:-1,weldTime:0,frame:0},this.mouse={x:-1e3,y:-1e3},this.dpr=Math.min(window.devicePixelRatio||1,3),this.themeColor="#003366",this.accentColor="#C69061",this.active=!0,this._resizeHandler=()=>this.resize(),this._mouseMoveHandler=a=>this.handleMouseMove(a),document.fonts?document.fonts.ready.then(()=>{this.active&&this.init()}):this.init(),this.animate(),window.addEventListener("resize",this._resizeHandler),window.addEventListener("mousemove",this._mouseMoveHandler),window.resetBrandBuild=()=>this.init())}init(){this.resize(),this.particles=[],this.sparks=[];const t=document.createElement("canvas"),a=t.getContext("2d"),i=2,s=1200*i,n=220*i;t.width=s,t.height=n,a.fillStyle="black",a.font=`bold ${100*i}px Inter, system-ui, sans-serif`,a.textAlign="center",a.textBaseline="middle",a.fillText("SAPTHAVARNAH",s/2,n/2);const r=a.getImageData(0,0,s,n).data,l=window.innerWidth<768,I=this.dpr>=2,y=l?10*i:I?5*i:6*i,p=this.canvas.width/this.dpr,g=this.canvas.height/this.dpr;for(let h=0;h<n;h+=y)for(let m=0;m<s;m+=y){const T=(h*s+m)*4;if(r[T+3]>128){const b=p/2-s/i/2+m/i,j=g*.82-n/i/2+h/i,w=Math.random()>.4,M=w?8:5;this.particles.push({tx:b,ty:j,x:b+(Math.random()-.5)*50,y:-Math.random()*1500-100,vx:0,vy:5+Math.random()*10,c:w?this.accentColor:this.themeColor,size:M,delay:Math.random()*150,landed:!1,welded:!1,bounce:0})}}this.worker={x:p/2,y:g,tx:p/2,ty:g*.8,state:"idle",targetIdx:-1,weldTime:0,frame:0,scale:1.8}}resize(){if(!this.canvas)return;const t=this.canvas.parentElement;this.canvas.width=t.offsetWidth*this.dpr,this.canvas.height=t.offsetHeight*this.dpr,this.canvas.style.width=t.offsetWidth+"px",this.canvas.style.height=t.offsetHeight+"px",this.ctx.setTransform(1,0,0,1,0,0),this.ctx.scale(this.dpr,this.dpr),this.ctx.imageSmoothingEnabled=!1}handleMouseMove(t){if(!this.canvas)return;const a=this.canvas.getBoundingClientRect();this.mouse.x=t.clientX-a.left,this.mouse.y=t.clientY-a.top}updateWorker(){if(this.worker.frame++,this.worker.state==="idle"||this.worker.state==="walking"){let t=1/0,a=-1;this.particles.forEach((i,s)=>{if(i.landed&&!i.welded){const n=i.tx-this.worker.x,r=i.ty-this.worker.y,l=n*n+r*r;l<t&&(t=l,a=s)}}),a!==-1&&(this.worker.targetIdx=a,this.worker.tx=this.particles[a].tx,this.worker.ty=this.particles[a].ty,this.worker.state="walking")}if(this.worker.state==="walking"){const t=this.worker.tx-this.worker.x,a=this.worker.ty-(this.worker.y-15);Math.sqrt(t*t+a*a)<10?(this.worker.state="welding",this.worker.weldTime=40):(this.worker.x+=t*.12,this.worker.y+=a*.12)}this.worker.state==="welding"&&(this.worker.weldTime>0?(this.worker.weldTime--,Math.random()>.5&&this.sparks.push({x:this.worker.x+10,y:this.worker.y-15,vx:(Math.random()-.5)*8,vy:-Math.random()*6-2,life:20+Math.random()*20})):(this.worker.targetIdx!==-1&&(this.particles[this.worker.targetIdx].welded=!0),this.worker.state="idle"))}drawWorker(){const{x:t,y:a,state:i,frame:s,scale:n}=this.worker,r=i==="walking"?Math.sin(s*.25)*4:0;this.ctx.save(),this.ctx.translate(t,a+r),this.ctx.scale(n,n),this.ctx.fillStyle="#1a202c",this.ctx.fillRect(-5,-6,3,6),this.ctx.fillRect(2,-6,3,6),this.ctx.fillStyle="#f6ad55",this.ctx.fillRect(-7,-22,14,16),this.ctx.fillStyle="rgba(255,255,255,0.9)",this.ctx.fillRect(-7,-18,14,2),this.ctx.fillRect(-7,-12,14,2),this.ctx.fillStyle="#4a5568",i==="welding"?this.ctx.fillRect(2,-18,12,3):(this.ctx.fillRect(7,-18,3,10),this.ctx.fillRect(-10,-18,3,10)),this.ctx.fillStyle="#f6e05e",this.ctx.beginPath(),this.ctx.arc(0,-25,6,Math.PI,0),this.ctx.fill(),this.ctx.fillRect(-8,-25,16,2),this.ctx.restore()}animate(){if(!this.active||!this.canvas)return;const t=this.canvas.width/this.dpr,a=this.canvas.height/this.dpr;this.ctx.clearRect(0,0,t,a),this.ctx.shadowBlur=0,this.particles.forEach(i=>{if(i.delay>0){i.delay--;return}i.landed?(i.x+=(i.tx-i.x)*.1,i.y+=(i.ty-i.y)*.1,i.bounce>0&&(i.y-=i.bounce,i.bounce*=-.5)):(i.y+=i.vy,i.vy+=.3,i.y>=i.ty&&(i.y=i.ty,i.landed=!0,i.bounce=6)),this.ctx.fillStyle=i.c,this.ctx.beginPath(),this.ctx.roundRect(i.x,i.y,i.size,i.size*.6,1),this.ctx.fill(),i.welded&&(this.ctx.fillStyle="rgba(255, 255, 255, 0.15)",this.ctx.fillRect(i.x,i.y,i.size,i.size*.15))}),this.sparks=this.sparks.filter(i=>{i.x+=i.vx,i.y+=i.vy,i.vy+=.3,i.life--;const s=i.life/40;return this.ctx.fillStyle=`rgba(255, 200, 50, ${s})`,this.ctx.beginPath(),this.ctx.arc(i.x,i.y,2,0,Math.PI*2),this.ctx.fill(),i.life>0}),this.updateWorker(),this.drawWorker(),requestAnimationFrame(()=>this.animate())}destroy(){this.active=!1,window.removeEventListener("resize",this._resizeHandler),window.removeEventListener("mousemove",this._mouseMoveHandler),this.canvas=null}}const D="/sapthavarna-web/".replace(/\/$/,"")||"",k=e=>`${D}/${e}`,S={header:()=>`
    <nav class="pill-nav">
    ...
    `};function q(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=S.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=S.footer(),E()}function E(){const e=window.location.pathname;document.querySelectorAll(".nav-link").forEach(t=>{const a=new URL(t.href,window.location.origin).pathname;a===e||e==="/"&&a==="/"?t.classList.add("active"):t.classList.remove("active")})}let d=null;C.initialize({startOnLoad:!1,theme:"base",themeVariables:{primaryColor:"#f6f5f2",primaryTextColor:"#1c1c1c",primaryBorderColor:"rgba(0,0,0,0.1)",lineColor:"#C69061",secondaryColor:"#f6f5f2",tertiaryColor:"#f6f5f2"}});const $=()=>{d&&(d.destroy(),d=null),document.getElementById("spectrum-canvas")&&(d=new L("spectrum-canvas"));const t={threshold:.05},a=new IntersectionObserver(s=>{s.forEach(n=>{n.isIntersecting&&(gsap.to(n.target,{opacity:1,y:0,duration:.8,ease:"power2.out"}),a.unobserve(n.target))})},t);document.querySelectorAll(".nm-card, h2, #closing h2, .subpage-hero h1, .hero-content h1").forEach(s=>{s.style.opacity="0",s.style.transform="translateY(15px)",a.observe(s)}),document.querySelectorAll(".mermaid").length>0&&C.run(),E()},G=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const a=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(a,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{q();const e=document.getElementById("app");new R(e),G(),window.addEventListener("routeChange",()=>{$()}),$()});window.setAnimationMode=e=>{d&&typeof d.setMode=="function"&&(d.setMode(e),document.querySelectorAll(".sample-btn").forEach(t=>{t.getAttribute("onclick").match(/'([^']+)'/)[1]===e?t.classList.add("active"):t.classList.remove("active")}))};window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,a=new FormData(t),i=Object.fromEntries(a.entries());i.submittedAt=new Date().toISOString();const s=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");s.push(i),localStorage.setItem("eoi_submissions",JSON.stringify(s)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"}),console.log("EOI Submitted:",i)};
