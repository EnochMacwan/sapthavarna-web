...
`).join(""),M=e=>e.map(t=>`<li>${t}</li>`).join(""),o={home:()=>{const e=c();return`
    ...
    `};function P(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=x.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=x.footer(),S()}function S(){const e=window.location.pathname;document.querySelectorAll(".nav-link").forEach(t=>{const a=new URL(t.href,window.location.origin).pathname;a===e||e==="/"&&a==="/"?t.classList.add("active"):t.classList.remove("active")})}let d=null;$.initialize({startOnLoad:!1,theme:"base",themeVariables:{primaryColor:"#f6f5f2",primaryTextColor:"#1c1c1c",primaryBorderColor:"rgba(0,0,0,0.1)",lineColor:"#C69061",secondaryColor:"#f6f5f2",tertiaryColor:"#f6f5f2"}});const k=()=>{d&&(d.destroy(),d=null),document.getElementById("spectrum-canvas")&&(d=new A("spectrum-canvas"));const t={threshold:.05},a=new IntersectionObserver(i=>{i.forEach(n=>{n.isIntersecting&&(gsap.to(n.target,{opacity:1,y:0,duration:.8,ease:"power2.out"}),a.unobserve(n.target))})},t);document.querySelectorAll(".nm-card, h2, #closing h2, .subpage-hero h1, .hero-content h1").forEach(i=>{i.style.opacity="0",i.style.transform="translateY(15px)",a.observe(i)}),document.querySelectorAll(".mermaid").length>0&&$.run(),S()},R=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const a=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(a,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{P();const e=document.getElementById("app");new j(e),R(),window.addEventListener("routeChange",()=>{k()}),k()});window.setAnimationMode=e=>{d&&typeof d.setMode=="function"&&(d.setMode(e),document.querySelectorAll(".sample-btn").forEach(t=>{t.getAttribute("onclick").match(/'([^']+)'/)[1]===e?t.classList.add("active"):t.classList.remove("active")}))};window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,a=new FormData(t),s=Object.fromEntries(a.entries());s.submittedAt=new Date().toISOString();const i=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");i.push(s),localStorage.setItem("eoi_submissions",JSON.stringify(i)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"}),console.log("EOI Submitted:",s)};
