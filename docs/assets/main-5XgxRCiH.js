...
`).join(""),R=e=>e.map(t=>`<li>${t}</li>`).join(""),o={home:()=>{const e=l();return`
        ...
        <section id="about">
            <div class="about-grid">
                ...
                <div class="nm-card img-card nm-inset">
                    <img src="${v("materials.png")}" alt="Engineering materials" class="brand-img">
                </div>
            </div>
        </section>
    ...
    `},about:()=>{const e=l();return`
        ...
        <section id="leadership">
            ...
            <div class="cards-grid team-grid">
                ${q(window.TEAM_DATA||e.about.leadershipTeam)}
            </div>
        </section>
        ...
        <section id="project-detail">
            <div class="detail-grid">
                <div class="detail-main">
                    ${s.image?`<div class="nm-card detail-image nm-inset"><img src="${v(s.image)}" alt="${s.title}"></div>`:""}
    ...
    `},teamDetail:(e,t)=>{const a=l(),i=(window.TEAM_DATA||a.about?.leadershipTeam||[]).find(n=>n.name.toLowerCase().replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,"")===e)||{name:"Team Member Not Found",role:"Unknown",desc:"The requested team member could not be found.",bio:"",photo:""};return`
        ...
        <section id="team-detail">
            <div class="team-detail-card nm-card">
                <div class="team-detail-photo">
                    ${i.photo?`<img src="${v(i.photo)}" alt="${i.name}">`:`<div class="team-avatar-xl">${i.name.charAt(0)}</div>`}
                </div>
    ...
    `}};class D{constructor(t){this.mountPoint=t,this.routes={"":o.home,"/":o.home,about:o.about,capabilities:o.capabilities,marine:o.marine,transport:o.transport,energy:o.energy,systems:o.systems,sustainability:o.sustainability,contact:o.contact,careers:o.careers},this.dynamicRoutes={project:o.projectDetail,service:o.serviceDetail,team:o.teamDetail},window.addEventListener("hashchange",()=>this.handleRoute()),document.addEventListener("click",a=>this.interceptClick(a)),this.handleRoute()}interceptClick(t){const a=t.target.closest("a");if(a&&a.classList.contains("nav-link")){const s=a.getAttribute("href");if(s&&s.startsWith("/")&&!s.startsWith("//")){t.preventDefault();const i=s==="/"?"":s.substring(1);window.location.hash=i}}}navigate(t){window.location.hash=t}getHashPath(){let t=window.location.hash.substring(1);return t.startsWith("/")&&(t=t.substring(1)),t}getQueryParams(t){const a={},s=t.indexOf("?");return s>-1&&t.substring(s+1).split("&").forEach(n=>{const[r,c]=n.split("=");a[decodeURIComponent(r)]=decodeURIComponent(c||"")}),a}async handleRoute(){let t=this.getHashPath();const a=this.getQueryParams(t),s=t.indexOf("?");s>-1&&(t=t.substring(0,s));let i;const n=Object.keys(this.dynamicRoutes).find(c=>t.startsWith(c));n&&a.id?i=()=>this.dynamicRoutes[n](a.id,a):i=this.routes[t]||o.home;const r=this.mountPoint;window.gsap&&await window.gsap.to(r,{opacity:0,y:10,duration:.3,ease:"power2.in"}),r.innerHTML=i(),window.scrollTo(0,0),window.gsap&&window.gsap.fromTo(r,{opacity:0,y:10},{opacity:1,y:0,duration:.5,ease:"power2.out"}),window.dispatchEvent(new CustomEvent("routeChange",{detail:{path:t,queryParams:a}}))}}class G{constructor(t){this.canvas=document.getElementById(t),this.canvas&&(this.ctx=this.canvas.getContext("2d",{alpha:!0}),this.particles=[],this.sparks=[],this.worker={x:0,y:0,tx:0,ty:0,state:"idle",targetIdx:-1,weldTime:0,frame:0},this.mouse={x:-1e3,y:-1e3},this.dpr=Math.min(window.devicePixelRatio||1,3),this.themeColor="#003366",this.accentColor="#C69061",this.active=!0,this._resizeHandler=()=>this.resize(),this._mouseMoveHandler=a=>this.handleMouseMove(a),document.fonts?document.fonts.ready.then(()=>{this.active&&this.init()}):this.init(),this.animate(),window.addEventListener("resize",this._resizeHandler),window.addEventListener("mousemove",this._mouseMoveHandler),window.resetBrandBuild=()=>this.init())}init(){this.resize(),this.particles=[],this.sparks=[];const t=document.createElement("canvas"),a=t.getContext("2d"),s=2,i=1200*s,n=220*s;t.width=i,t.height=n,a.fillStyle="black",a.font=`bold ${100*s}px Inter, system-ui, sans-serif`,a.textAlign="center",a.textBaseline="middle",a.fillText("SAPTHAVARNAH",i/2,n/2);const r=a.getImageData(0,0,i,n).data,c=window.innerWidth<768,f=this.dpr>=2,m=c?10*s:f?5*s:6*s,p=this.canvas.width/this.dpr,g=this.canvas.height/this.dpr;for(let d=0;d<n;d+=m)for(let h=0;h<i;h+=m){const b=(d*i+h)*4;if(r[b+3]>128){const k=p/2-i/s/2+h/s,j=g*.82-n/s/2+d/s,x=Math.random()>.4,P=x?8:5;this.particles.push({tx:k,ty:j,x:k+(Math.random()-.5)*50,y:-Math.random()*1500-100,vx:0,vy:5+Math.random()*10,c:x?this.accentColor:this.themeColor,size:P,delay:Math.random()*150,landed:!1,welded:!1,bounce:0})}}this.worker={x:p/2,y:g,tx:p/2,ty:g*.8,state:"idle",targetIdx:-1,weldTime:0,frame:0,scale:1.8}}resize(){if(!this.canvas)return;const t=this.canvas.parentElement;this.canvas.width=t.offsetWidth*this.dpr,this.canvas.height=t.offsetHeight*this.dpr,this.canvas.style.width=t.offsetWidth+"px",this.canvas.style.height=t.offsetHeight+"px",this.ctx.setTransform(1,0,0,1,0,0),this.ctx.scale(this.dpr,this.dpr),this.ctx.imageSmoothingEnabled=!1}handleMouseMove(t){if(!this.canvas)return;const a=this.canvas.getBoundingClientRect();this.mouse.x=t.clientX-a.left,this.mouse.y=t.clientY-a.top}updateWorker(){if(this.worker.frame++,this.worker.state==="idle"||this.worker.state==="walking"){let t=1/0,a=-1;this.particles.forEach((s,i)=>{if(s.landed&&!s.welded){const n=s.tx-this.worker.x,r=s.ty-this.worker.y,c=n*n+r*r;c<t&&(t=c,a=i)}}),a!==-1&&(this.worker.targetIdx=a,this.worker.tx=this.particles[a].tx,this.worker.ty=this.particles[a].ty,this.worker.state="walking")}if(this.worker.state==="walking"){const t=this.worker.tx-this.worker.x,a=this.worker.ty-(this.worker.y-15);Math.sqrt(t*t+a*a)<10?(this.worker.state="welding",this.worker.weldTime=40):(this.worker.x+=t*.12,this.worker.y+=a*.12)}this.worker.state==="welding"&&(this.worker.weldTime>0?(this.worker.weldTime--,Math.random()>.5&&this.sparks.push({x:this.worker.x+10,y:this.worker.y-15,vx:(Math.random()-.5)*8,vy:-Math.random()*6-2,life:20+Math.random()*20})):(this.worker.targetIdx!==-1&&(this.particles[this.worker.targetIdx].welded=!0),this.worker.state="idle"))}drawWorker(){const{x:t,y:a,state:s,frame:i,scale:n}=this.worker,r=s==="walking"?Math.sin(i*.25)*4:0;this.ctx.save(),this.ctx.translate(t,a+r),this.ctx.scale(n,n),this.ctx.fillStyle="#1a202c",this.ctx.fillRect(-5,-6,3,6),this.ctx.fillRect(2,-6,3,6),this.ctx.fillStyle="#f6ad55",this.ctx.fillRect(-7,-22,14,16),this.ctx.fillStyle="rgba(255,255,255,0.9)",this.ctx.fillRect(-7,-18,14,2),this.ctx.fillRect(-7,-12,14,2),this.ctx.fillStyle="#4a5568",s==="welding"?this.ctx.fillRect(2,-18,12,3):(this.ctx.fillRect(7,-18,3,10),this.ctx.fillRect(-10,-18,3,10)),this.ctx.fillStyle="#f6e05e",this.ctx.beginPath(),this.ctx.arc(0,-25,6,Math.PI,0),this.ctx.fill(),this.ctx.fillRect(-8,-25,16,2),this.ctx.restore()}animate(){if(!this.active||!this.canvas)return;const t=this.canvas.width/this.dpr,a=this.canvas.height/this.dpr;this.ctx.clearRect(0,0,t,a),this.ctx.shadowBlur=0,this.particles.forEach(s=>{if(s.delay>0){s.delay--;return}s.landed?(s.x+=(s.tx-s.x)*.1,s.y+=(s.ty-s.y)*.1,s.bounce>0&&(s.y-=s.bounce,s.bounce*=-.5)):(s.y+=s.vy,s.vy+=.3,s.y>=s.ty&&(s.y=s.ty,s.landed=!0,s.bounce=6)),this.ctx.fillStyle=s.c,this.ctx.beginPath(),this.ctx.roundRect(s.x,s.y,s.size,s.size*.6,1),this.ctx.fill(),s.welded&&(this.ctx.fillStyle="rgba(255, 255, 255, 0.15)",this.ctx.fillRect(s.x,s.y,s.size,s.size*.15))}),this.sparks=this.sparks.filter(s=>{s.x+=s.vx,s.y+=s.vy,s.vy+=.3,s.life--;const i=s.life/40;return this.ctx.fillStyle=`rgba(255, 200, 50, ${i})`,this.ctx.beginPath(),this.ctx.arc(s.x,s.y,2,0,Math.PI*2),this.ctx.fill(),s.life>0}),this.updateWorker(),this.drawWorker(),requestAnimationFrame(()=>this.animate())}destroy(){this.active=!1,window.removeEventListener("resize",this._resizeHandler),window.removeEventListener("mousemove",this._mouseMoveHandler),this.canvas=null}}const I="/sapthavarna-web/".replace(/\/$/,"")||"",$=e=>`${I}/${e}`;function W(){const e=window.NAV_DATA;if(!e)return null;let t="";return(e.megaMenus||[]).forEach(a=>{t+=`<div class="nav-item-has-mega">
            <a href="${a.href}" class="nav-link">${a.label}</a>
            <div class="mega-menu">`,(a.columns||[]).forEach(s=>{t+=`<div class="mega-col"><h5>${s.heading}</h5>`,s.type==="text"?t+=`<p class="mega-desc">${s.text}</p>`:(t+='<ul class="mega-links">',(s.links||[]).forEach(i=>{t+=`<li><a href="${i.href}">${i.text}</a></li>`}),t+="</ul>"),t+="</div>"}),t+="</div></div>"}),(e.standaloneLinks||[]).forEach(a=>{t+=`<a href="${a.href}" class="nav-link">${a.label}</a>`}),t}const E={header:()=>{const t=W()||`
    ...
    <footer class="main-footer">
        ...
        <div class="footer-bottom">
            ...
            <div class="legal-links">
                ...
                <a href="${I}/admin.html" class="admin-link">Admin</a>
            </div>
        </div>
    </footer>
    `};function B(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=E.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=E.footer(),M()}function M(){const e=window.location.hash.replace("#","").split("?")[0];document.querySelectorAll(".nav-link").forEach(t=>{const a=t.getAttribute("href")||"",s=a.startsWith("#")?a.substring(1):"";e===s||e===""&&s===""?t.classList.add("active"):t.classList.remove("active")})}let u=null;T.initialize({startOnLoad:!1,theme:"base",themeVariables:{primaryColor:"#f6f5f2",primaryTextColor:"#1c1c1c",primaryBorderColor:"rgba(0,0,0,0.1)",lineColor:"#C69061",secondaryColor:"#f6f5f2",tertiaryColor:"#f6f5f2"}});const C=()=>{u&&(u.destroy(),u=null),document.getElementById("spectrum-canvas")&&(u=new G("spectrum-canvas"));const t={threshold:.05},a=new IntersectionObserver(i=>{i.forEach(n=>{n.isIntersecting&&(gsap.to(n.target,{opacity:1,y:0,duration:.8,ease:"power2.out"}),a.unobserve(n.target))})},t);document.querySelectorAll(".nm-card, h2, #closing h2, .subpage-hero h1, .hero-content h1").forEach(i=>{i.style.opacity="0",i.style.transform="translateY(15px)",a.observe(i)}),document.querySelectorAll(".mermaid").length>0&&T.run(),M()},H=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const a=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(a,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{B();const e=document.getElementById("app");new D(e),H(),window.addEventListener("routeChange",()=>{C()}),C()});window.setAnimationMode=e=>{u&&typeof u.setMode=="function"&&(u.setMode(e),document.querySelectorAll(".sample-btn").forEach(t=>{t.getAttribute("onclick").match(/'([^']+)'/)[1]===e?t.classList.add("active"):t.classList.remove("active")}))};window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,a=new FormData(t),s=Object.fromEntries(a.entries());s.submittedAt=new Date().toISOString();const i=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");i.push(s),localStorage.setItem("eoi_submissions",JSON.stringify(i)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"}),console.log("EOI Submitted:",s)};const V=()=>{const e=document.createElement("button");e.className="back-to-top",e.setAttribute("aria-label","Back to top"),e.innerHTML='<i class="fas fa-arrow-up"></i>',document.body.appendChild(e),e.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}),window.addEventListener("scroll",()=>{window.scrollY>400?e.classList.add("visible"):e.classList.remove("visible")})},F=()=>{const e=document.querySelector("nav");if(!e)return;const t=document.createElement("div");t.className="hamburger",t.innerHTML="<span></span><span></span><span></span>",t.setAttribute("aria-label","Toggle menu");const a=document.createElement("div");a.className="mobile-nav",a.innerHTML=`
    ...
    `;const s=e.querySelector(".nav-container");s&&s.appendChild(t),document.body.appendChild(a),t.addEventListener("click",()=>{t.classList.toggle("active"),a.classList.toggle("active"),document.body.style.overflow=a.classList.contains("active")?"hidden":""}),a.querySelectorAll("a").forEach(i=>{i.addEventListener("click",()=>{t.classList.remove("active"),a.classList.remove("active"),document.body.style.overflow=""})})},N=()=>{const e=document.createElement("button");e.className="theme-toggle",e.setAttribute("aria-label","Toggle dark mode"),e.innerHTML='<i class="fas fa-moon"></i>',document.body.appendChild(e),localStorage.getItem("theme")==="dark"&&(document.body.classList.add("dark-mode"),e.innerHTML='<i class="fas fa-sun"></i>'),e.addEventListener("click",()=>{document.body.classList.toggle("dark-mode");const a=document.body.classList.contains("dark-mode");localStorage.setItem("theme",a?"dark":"light"),e.innerHTML=a?'<i class="fas fa-sun"></i>':'<i class="fas fa-moon"></i>'})},A=()=>{const e=document.querySelectorAll(".stat-number");if(e.length===0)return;const t=s=>{const i=parseInt(s.getAttribute("data-target"))||0,n=s.getAttribute("data-suffix")||"",r=2e3,c=0,f=performance.now(),m=p=>{const g=p-f,d=Math.min(g/r,1),h=1-Math.pow(1-d,3),b=Math.floor(c+(i-c)*h);s.textContent=b.toLocaleString()+n,d<1&&requestAnimationFrame(m)};requestAnimationFrame(m)},a=new IntersectionObserver(s=>{s.forEach(i=>{i.isIntersecting&&!i.target.classList.contains("animated")&&(i.target.classList.add("animated"),t(i.target))})},{threshold:.5});e.forEach(s=>a.observe(s))},L=()=>{document.querySelectorAll(".form-input").forEach(e=>{e.addEventListener("blur",()=>{if(e.required&&!e.value.trim())e.classList.add("invalid"),e.classList.remove("valid");else if(e.type==="email"&&e.value){const t=/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(e.value);e.classList.toggle("valid",t),e.classList.toggle("invalid",!t)}else e.value.trim()&&(e.classList.add("valid"),e.classList.remove("invalid"))}),e.addEventListener("input",()=>{e.classList.remove("invalid")})})};document.addEventListener("DOMContentLoaded",()=>{V(),F(),N(),A(),L()});window.addEventListener("routeChange",()=>{A(),L()});
