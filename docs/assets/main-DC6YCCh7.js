...
`).join(""),P=e=>e.map(t=>`<li>${t}</li>`).join(""),r={home:()=>{const e=c();return`
    ...
    `},about:()=>{const e=c();return`
        ...
        <section id="leadership">
            ...
            <div class="cards-grid team-grid">
                ${O(window.TEAM_DATA||e.about.leadershipTeam)}
            </div>
        </section>
    ...
    `};function q(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=k.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=k.footer(),C()}function C(){const e=window.location.hash.replace("#","").split("?")[0];document.querySelectorAll(".nav-link").forEach(t=>{const i=t.getAttribute("href")||"",s=i.startsWith("#")?i.substring(1):"";e===s||e===""&&s===""?t.classList.add("active"):t.classList.remove("active")})}let h=null;const S=()=>{h&&(h.destroy(),h=null),document.getElementById("spectrum-canvas")&&(h=new G("spectrum-canvas"));const t={threshold:.05},i=new IntersectionObserver(s=>{s.forEach(a=>{a.isIntersecting&&(gsap.to(a.target,{opacity:1,y:0,duration:.8,ease:"power2.out"}),i.unobserve(a.target))})},t);document.querySelectorAll(".nm-card, h2, #closing h2, .subpage-hero h1, .hero-content h1").forEach(s=>{s.style.opacity="0",s.style.transform="translateY(15px)",i.observe(s)}),C()},D=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const i=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(i,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{q();const e=document.getElementById("app");new R(e),D(),window.addEventListener("routeChange",()=>{S()}),S()});window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,i=new FormData(t),s=Object.fromEntries(i.entries());s.submittedAt=new Date().toISOString();const a=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");a.push(s),localStorage.setItem("eoi_submissions",JSON.stringify(a)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"})};const B=()=>{const e=document.createElement("button");e.className="back-to-top",e.setAttribute("aria-label","Back to top"),e.innerHTML='<i class="fas fa-arrow-up"></i>',document.body.appendChild(e),e.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}),window.addEventListener("scroll",()=>{window.scrollY>400?e.classList.add("visible"):e.classList.remove("visible")})},H=()=>{const e=document.querySelector("nav");if(!e)return;const t=document.createElement("div");t.className="hamburger",t.innerHTML="<span></span><span></span><span></span>",t.setAttribute("aria-label","Toggle menu");const i=document.createElement("div");i.className="mobile-nav",i.innerHTML=`
