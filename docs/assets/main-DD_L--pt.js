...
`).join(""),R=e=>e.map(t=>`<li>${t}</li>`).join(""),B=e=>!e||!e.length?"":`
    ...
    <section class="section-alt">
        ...
        ${n("home","stats")?B(e.home.stats):""}

        ${n("home","clientLogos")?F(e.home.clientLogos):""}
        ...
        ${n("home","process")?j(e.home.process):""}

        ${n("home","testimonials")?V(e.home.testimonials):""}

        ${n("home","trustStrip")?N(e.home.trustStrip):""}
    ...
    `},capabilities:()=>{const e=l();return`
        ...
        ${n("capabilities","methodology")?j(e.capabilities.methodology):""}
    ...
    `},marine:()=>{const e=l();return`
        ...
        ${n("marine","sectorStats")?f(e.marine.sectorStats):""}
        ...
        ${n("marine","whyChooseUs")?b(e.marine.whyChooseUs):""}
    ...
    `},transport:()=>{const e=l();return`
        ...
        ${n("transport","sectorStats")?f(e.transport.sectorStats):""}
        ...
        ${n("transport","whyChooseUs")?b(e.transport.whyChooseUs):""}
    ...
    `},energy:()=>{const e=l();return`
        ...
        ${n("energy","sectorStats")?f(e.energy.sectorStats):""}
        ...
        ${n("energy","whyChooseUs")?b(e.energy.whyChooseUs):""}
    ...
    `},systems:()=>{const e=l();return`
        ...
        ${n("systems","sectorStats")?f(e.systems.sectorStats):""}
        ...
        ${n("systems","whyChooseUs")?b(e.systems.whyChooseUs):""}
    ...
    `};class U{constructor(t){this.mountPoint=t,this._routeVersion=0,this.routes={"":c.home,"/":c.home,about:c.about,capabilities:c.capabilities,marine:c.marine,transport:c.transport,energy:c.energy,systems:c.systems,sustainability:c.sustainability,contact:c.contact,careers:c.careers},window.addEventListener("hashchange",()=>this.handleRoute()),document.addEventListener("click",i=>this.interceptClick(i)),this.handleRoute()}interceptClick(t){const i=t.target.closest("a");if(i&&i.classList.contains("nav-link")){const s=i.getAttribute("href");if(s&&s.startsWith("/")&&!s.startsWith("//")){t.preventDefault();const a=s==="/"?"":s.substring(1);window.location.hash=a}}}navigate(t){window.location.hash=t}getHashPath(){let t=window.location.hash.substring(1);return t.startsWith("/")&&(t=t.substring(1)),t}async handleRoute(){const t=++this._routeVersion,i=this.mountPoint;if(window.gsap&&await window.gsap.to(i,{opacity:0,y:10,duration:.3,ease:"power2.in"}),t!==this._routeVersion)return;const s=this.getHashPath(),a=this.routes[s]||c.home;i.innerHTML=a(),window.scrollTo(0,0),window.gsap&&window.gsap.fromTo(i,{opacity:0,y:10},{opacity:1,y:0,duration:.5,ease:"power2.out"}),window.dispatchEvent(new CustomEvent("routeChange",{detail:{path:s}}))}}class z{constructor(t){this.canvas=document.getElementById(t),this.canvas&&(this.ctx=this.canvas.getContext("2d",{alpha:!0}),this.particles=[],this.sparks=[],this.worker={x:0,y:0,tx:0,ty:0,state:"idle",targetIdx:-1,weldTime:0,frame:0},this.mouse={x:-1e3,y:-1e3},this.dpr=Math.min(window.devicePixelRatio||1,3),this.themeColor="#003366",this.accentColor="#C69061",this.active=!0,this._resizeHandler=()=>this.resize(),this._mouseMoveHandler=i=>this.handleMouseMove(i),document.fonts?document.fonts.ready.then(()=>{this.active&&this.init()}):this.init(),this.animate(),window.addEventListener("resize",this._resizeHandler),window.addEventListener("mousemove",this._mouseMoveHandler),window.resetBrandBuild=()=>this.init())}init(){this.resize(),this.particles=[],this.sparks=[];const t=document.createElement("canvas"),i=t.getContext("2d"),s=2,a=1200*s,r=220*s;t.width=a,t.height=r,i.fillStyle="black",i.font=`bold ${100*s}px Inter, system-ui, sans-serif`,i.textAlign="center",i.textBaseline="middle",i.fillText("SAPTHAVARNAH",a/2,r/2);const o=i.getImageData(0,0,a,r).data,d=window.innerWidth<768,p=this.dpr>=2,u=d?10*s:p?5*s:6*s,$=this.canvas.width/this.dpr,x=this.canvas.height/this.dpr;for(let v=0;v<r;v+=u)for(let g=0;g<a;g+=u){const X=(v*a+g)*4;if(o[X+3]>128){const S=$/2-a/s/2+g/s,q=x*.82-r/s/2+v/s,C=Math.random()>.4,W=C?8:5;this.particles.push({tx:S,ty:q,x:S+(Math.random()-.5)*50,y:-Math.random()*1500-100,vx:0,vy:5+Math.random()*10,c:C?this.accentColor:this.themeColor,size:W,delay:Math.random()*150,landed:!1,welded:!1,bounce:0})}}this.worker={x:$/2,y:x,tx:$/2,ty:x*.8,state:"idle",targetIdx:-1,weldTime:0,frame:0,scale:1.8}}resize(){if(!this.canvas)return;const t=this.canvas.parentElement;this.canvas.width=t.offsetWidth*this.dpr,this.canvas.height=t.offsetHeight*this.dpr,this.canvas.style.width=t.offsetWidth+"px",this.canvas.style.height=t.offsetHeight+"px",this.ctx.setTransform(1,0,0,1,0,0),this.ctx.scale(this.dpr,this.dpr),this.ctx.imageSmoothingEnabled=!1}handleMouseMove(t){if(!this.canvas)return;const i=this.canvas.getBoundingClientRect();this.mouse.x=t.clientX-i.left,this.mouse.y=t.clientY-i.top}updateWorker(){if(this.worker.frame++,this.worker.state==="idle"||this.worker.state==="walking"){let t=1/0,i=-1;this.particles.forEach((s,a)=>{if(s.landed&&!s.welded){const r=s.tx-this.worker.x,o=s.ty-this.worker.y,d=r*r+o*o;d<t&&(t=d,i=a)}}),i!==-1&&(this.worker.targetIdx=i,this.worker.tx=this.particles[i].tx,this.worker.ty=this.particles[i].ty,this.worker.state="walking")}if(this.worker.state==="walking"){const t=this.worker.tx-this.worker.x,i=this.worker.ty-(this.worker.y-15);Math.sqrt(t*t+i*i)<10?(this.worker.state="welding",this.worker.weldTime=40):(this.worker.x+=t*.12,this.worker.y+=i*.12)}this.worker.state==="welding"&&(this.worker.weldTime>0?(this.worker.weldTime--,Math.random()>.5&&this.sparks.push({x:this.worker.x+10,y:this.worker.y-15,vx:(Math.random()-.5)*8,vy:-Math.random()*6-2,life:20+Math.random()*20})):(this.worker.targetIdx!==-1&&(this.particles[this.worker.targetIdx].welded=!0),this.worker.state="idle"))}drawWorker(){const{x:t,y:i,state:s,frame:a,scale:r}=this.worker,o=s==="walking"?Math.sin(a*.25)*4:0;this.ctx.save(),this.ctx.translate(t,i+o),this.ctx.scale(r,r),this.ctx.fillStyle="#1a202c",this.ctx.fillRect(-5,-6,3,6),this.ctx.fillRect(2,-6,3,6),this.ctx.fillStyle="#f6ad55",this.ctx.fillRect(-7,-22,14,16),this.ctx.fillStyle="rgba(255,255,255,0.9)",this.ctx.fillRect(-7,-18,14,2),this.ctx.fillRect(-7,-12,14,2),this.ctx.fillStyle="#4a5568",s==="welding"?this.ctx.fillRect(2,-18,12,3):(this.ctx.fillRect(7,-18,3,10),this.ctx.fillRect(-10,-18,3,10)),this.ctx.fillStyle="#f6e05e",this.ctx.beginPath(),this.ctx.arc(0,-25,6,Math.PI,0),this.ctx.fill(),this.ctx.fillRect(-8,-25,16,2),this.ctx.restore()}animate(){if(!this.active||!this.canvas)return;const t=this.canvas.width/this.dpr,i=this.canvas.height/this.dpr;this.ctx.clearRect(0,0,t,i),this.ctx.shadowBlur=0,this.particles.forEach(s=>{if(s.delay>0){s.delay--;return}s.landed?(s.x+=(s.tx-s.x)*.1,s.y+=(s.ty-s.y)*.1,s.bounce>0&&(s.y-=s.bounce,s.bounce*=-.5)):(s.y+=s.vy,s.vy+=.3,s.y>=s.ty&&(s.y=s.ty,s.landed=!0,s.bounce=6)),this.ctx.fillStyle=s.c,this.ctx.beginPath(),this.ctx.roundRect(s.x,s.y,s.size,s.size*.6,1),this.ctx.fill(),s.welded&&(this.ctx.fillStyle="rgba(255, 255, 255, 0.15)",this.ctx.fillRect(s.x,s.y,s.size,s.size*.15))}),this.sparks=this.sparks.filter(s=>{s.x+=s.vx,s.y+=s.vy,s.vy+=.3,s.life--;const a=s.life/40;return this.ctx.fillStyle=`rgba(255, 200, 50, ${a})`,this.ctx.beginPath(),this.ctx.arc(s.x,s.y,2,0,Math.PI*2),this.ctx.fill(),s.life>0}),this.updateWorker(),this.drawWorker(),requestAnimationFrame(()=>this.animate())}destroy(){this.active=!1,window.removeEventListener("resize",this._resizeHandler),window.removeEventListener("mousemove",this._mouseMoveHandler),this.canvas=null}}const M="/sapthavarna-web/".replace(/\/$/,"")||"",I=e=>`${M}/${e}`;function _(){const e=window.NAV_DATA;if(!e)return null;let t="";return(e.megaMenus||[]).forEach(i=>{t+=`<div class="nav-item-has-mega">
            <a href="${i.href}" class="nav-link">${i.label}</a>
            <div class="mega-menu">`,(i.columns||[]).forEach(s=>{t+=`<div class="mega-col"><h5>${s.heading}</h5>`,s.type==="text"?t+=`<p class="mega-desc">${s.text}</p>`:(t+='<ul class="mega-links">',(s.links||[]).forEach(a=>{t+=`<li><a href="${a.href}">${a.text}</a></li>`}),t+="</ul>"),t+="</div>"}),t+="</div></div>"}),(e.standaloneLinks||[]).forEach(i=>{t+=`<a href="${i.href}" class="nav-link">${i.label}</a>`}),t}const L={header:()=>{const t=_()||`
    ...
    `}};function Y(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=L.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=L.footer(),A()}function A(){const e=window.location.hash.replace("#","").split("?")[0];document.querySelectorAll(".nav-link").forEach(t=>{const i=t.getAttribute("href")||"",s=i.startsWith("#")?i.substring(1):"";e===s||e===""&&s===""?t.classList.add("active"):t.classList.remove("active")})}let y=null,m=null;const P=()=>{y&&(y.destroy(),y=null),m&&(m.disconnect(),m=null),document.getElementById("spectrum-canvas")&&(y=new z("spectrum-canvas"));const t={threshold:.05,rootMargin:"0px 0px -30px 0px"};m=new IntersectionObserver(s=>{s.forEach(a=>{a.isIntersecting&&(gsap.to(a.target,{opacity:1,y:0,duration:.7,ease:"power2.out",delay:a.target.dataset.delay||0}),m.unobserve(a.target))})},t),document.querySelectorAll(".nm-card, .grid-card-wrapper").forEach(s=>{gsap.set(s,{opacity:0,y:15});const a=s.parentElement;if(a&&(a.classList.contains("cards-grid")||a.classList.contains("careers-grid")||a.classList.contains("benefits-grid")||a.classList.contains("team-grid")||a.classList.contains("stats-bar")||a.classList.contains("process-grid")||a.classList.contains("certs-grid")||a.classList.contains("why-grid")||a.classList.contains("testimonials-grid")||a.classList.contains("projects-grid"))){const o=Array.from(a.children).indexOf(s);s.dataset.delay=(o*.06).toFixed(2)}m.observe(s)}),A()},J=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const i=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(i,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{Y();const e=document.getElementById("app");new U(e),J(),window.addEventListener("routeChange",()=>{P(),T()}),P(),T()});window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,i=new FormData(t),s=Object.fromEntries(i.entries());s.submittedAt=new Date().toISOString();const a=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");a.push(s),localStorage.setItem("eoi_submissions",JSON.stringify(a)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"})};const T=()=>{const e=document.getElementById("contact-form");e&&(e.addEventListener("submit",t=>{t.preventDefault();let i=!0;const s=document.getElementById("contact-name"),a=document.getElementById("contact-email"),r=document.getElementById("contact-message");if([s,a,r].forEach(u=>{u.classList.remove("valid","invalid"),u.value.trim()?u.classList.add("valid"):(u.classList.add("invalid"),i=!1)}),a.value&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a.value)&&(a.classList.remove("valid"),a.classList.add("invalid"),i=!1),!i)return;const o={name:s.value,company:document.getElementById("contact-company").value,email:a.value,phone:document.getElementById("contact-phone").value,sector:document.getElementById("contact-sector").value,region:document.getElementById("contact-region").value,message:r.value,submittedAt:new Date().toISOString()},d=JSON.parse(localStorage.getItem("contact_submissions")||"[]");d.push(o),localStorage.setItem("contact_submissions",JSON.stringify(d)),e.style.display="none";const p=document.getElementById("contact-success");p.style.display="block",p.scrollIntoView({behavior:"smooth",block:"center"})}),e.querySelectorAll(".form-input[required]").forEach(t=>{t.addEventListener("blur",()=>{t.classList.remove("valid","invalid"),t.value.trim()&&(t.type==="email"&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t.value)?t.classList.add("invalid"):t.classList.add("valid"))})}))},K=()=>{const e=document.createElement("button");e.className="back-to-top",e.setAttribute("aria-label","Back to top"),e.innerHTML='<i class="fas fa-arrow-up"></i>',document.body.appendChild(e),e.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}),window.addEventListener("scroll",()=>{window.scrollY>400?e.classList.add("visible"):e.classList.remove("visible")})},Q=()=>{const e=document.querySelector("nav");if(!e)return;const t=document.createElement("div");t.className="hamburger",t.innerHTML="<span></span><span></span><span></span>",t.setAttribute("aria-label","Toggle menu");const i=document.createElement("div");i.className="mobile-nav",i.innerHTML=`
