...
`).join(""),P=e=>e.map(t=>`<li>${t}</li>`).join(""),o={home:()=>{const e=c();return`
        ...
        <section id="about" class="section-alt">
            ...
            <div class="about-grid">
                ...
                <div class="nm-card img-card nm-inset">
                    <img src="${k("materials.png")}" alt="Engineering materials" class="brand-img">
                </div>
            </div>
        </section>
        ...
        <section id="technologies">
            ...
            <div class="cards-grid">
                ${e.systems.technologies.map((t,i)=>`
                    <div class="nm-card">
                        ${t.image?`<div class="card-image nm-inset mb-4"><img src="${k(t.image)}" alt="${t.title}" class="brand-img" style="height: 200px;"></div>`:""}
                        <div class="service-number">${String(i+1).padStart(2,"0")}</div>
                        <h3>${t.title}</h3>
                        <p class="text-secondary mt-4">${t.desc}</p>
                    </div>
    ...
    <nav class="pill-nav">
        <div class="nav-container">
            <a href="#" class="logo nav-link" aria-label="SapthaVarnah Home">
                <div class="logo-crop">
                    <img src="${S("logo-master.png")}" alt="SapthaVarnah Logo" class="logo-img-master">
                </div>
    ...
    `};function B(){let e=document.getElementById("header-mount");e||(e=document.createElement("div"),e.id="header-mount",document.body.insertBefore(e,document.body.firstChild)),e.innerHTML=E.header();let t=document.getElementById("footer-mount");t||(t=document.createElement("div"),t.id="footer-mount",document.body.appendChild(t)),t.innerHTML=E.footer(),T()}function T(){const e=window.location.hash.replace("#","").split("?")[0];document.querySelectorAll(".nav-link").forEach(t=>{const i=t.getAttribute("href")||"",s=i.startsWith("#")?i.substring(1):"";e===s||e===""&&s===""?t.classList.add("active"):t.classList.remove("active")})}let v=null,u=null;const $=()=>{v&&(v.destroy(),v=null),u&&(u.disconnect(),u=null),document.getElementById("spectrum-canvas")&&(v=new G("spectrum-canvas"));const t={threshold:.05,rootMargin:"0px 0px -30px 0px"};u=new IntersectionObserver(s=>{s.forEach(a=>{a.isIntersecting&&(gsap.to(a.target,{opacity:1,y:0,duration:.7,ease:"power2.out",delay:a.target.dataset.delay||0}),u.unobserve(a.target))})},t),document.querySelectorAll(".nm-card, .grid-card-wrapper").forEach(s=>{gsap.set(s,{opacity:0,y:15});const a=s.parentElement;if(a&&(a.classList.contains("cards-grid")||a.classList.contains("careers-grid")||a.classList.contains("benefits-grid")||a.classList.contains("team-grid"))){const r=Array.from(a.children).indexOf(s);s.dataset.delay=(r*.06).toFixed(2)}u.observe(s)}),T()},D=()=>{document.querySelectorAll(".nav-item-has-mega").forEach(t=>{const i=t.querySelectorAll(".mega-links li");t.addEventListener("mouseenter",()=>{gsap.fromTo(i,{opacity:0,x:-5},{opacity:1,x:0,stagger:.05,duration:.4,ease:"power2.out",delay:.1})})}),gsap.from("nav",{y:-10,opacity:0,duration:.8,ease:"power2.out"})};document.addEventListener("DOMContentLoaded",()=>{B();const e=document.getElementById("app");new W(e),D(),window.addEventListener("routeChange",()=>{$(),C()}),$(),C()});window.handleEOISubmit=e=>{e.preventDefault();const t=e.target,i=new FormData(t),s=Object.fromEntries(i.entries());s.submittedAt=new Date().toISOString();const a=JSON.parse(localStorage.getItem("eoi_submissions")||"[]");a.push(s),localStorage.setItem("eoi_submissions",JSON.stringify(a)),t.style.display="none",document.getElementById("eoi-success").style.display="block",document.getElementById("eoi-success").scrollIntoView({behavior:"smooth",block:"center"})};const C=()=>{const e=document.getElementById("contact-form");e&&(e.addEventListener("submit",t=>{t.preventDefault();let i=!0;const s=document.getElementById("contact-name"),a=document.getElementById("contact-email"),n=document.getElementById("contact-message");if([s,a,n].forEach(d=>{d.classList.remove("valid","invalid"),d.value.trim()?d.classList.add("valid"):(d.classList.add("invalid"),i=!1)}),a.value&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(a.value)&&(a.classList.remove("valid"),a.classList.add("invalid"),i=!1),!i)return;const r={name:s.value,company:document.getElementById("contact-company").value,email:a.value,phone:document.getElementById("contact-phone").value,sector:document.getElementById("contact-sector").value,region:document.getElementById("contact-region").value,message:n.value,submittedAt:new Date().toISOString()},l=JSON.parse(localStorage.getItem("contact_submissions")||"[]");l.push(r),localStorage.setItem("contact_submissions",JSON.stringify(l)),e.style.display="none";const m=document.getElementById("contact-success");m.style.display="block",m.scrollIntoView({behavior:"smooth",block:"center"})}),e.querySelectorAll(".form-input[required]").forEach(t=>{t.addEventListener("blur",()=>{t.classList.remove("valid","invalid"),t.value.trim()&&(t.type==="email"&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t.value)?t.classList.add("invalid"):t.classList.add("valid"))})}))},V=()=>{const e=document.createElement("button");e.className="back-to-top",e.setAttribute("aria-label","Back to top"),e.innerHTML='<i class="fas fa-arrow-up"></i>',document.body.appendChild(e),e.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}),window.addEventListener("scroll",()=>{window.scrollY>400?e.classList.add("visible"):e.classList.remove("visible")})},H=()=>{const e=document.querySelector("nav");if(!e)return;const t=document.createElement("div");t.className="hamburger",t.innerHTML="<span></span><span></span><span></span>",t.setAttribute("aria-label","Toggle menu");const i=document.createElement("div");i.className="mobile-nav",i.innerHTML=`
